...
export class FilterService {
  ...
  constructor(private http: HttpClient, private pagination: PaginationService) {
    this.pagination
      ...
      .subscribe((pageNumber) => (this.currentPage = pageNumber));
  }
    ...
    switch (type) {
      case filterTypes.preparationType: {
        if (!this.preparationTypes.includes(filterValue)) {
          ...
          this.preparationTypes = this.preparationTypes.filter(
            (preparationType) => {
              if (preparationType != filterValue) {
                return true;
              } else return false;
            }
          );
  ...
  search(addToRecipes: boolean) {
    let body: any = {};
    body.preparationTypes =
      this.preparationTypes.length > 0 ? this.preparationTypes : [''];
    body.ingredients = this.ingredients.length > 0 ? this.ingredients : [''];
    body.difficultyLevel =
      this.difficultyLevel.length > 0 ? this.difficultyLevel : [''];
    ...
    this.http.post(searchUrl, body).subscribe((response) => {
      ...
      if (addToRecipes) {
        this.getRecipeUpdate().subscribe(
          (current) => (this.currentRecipes = current)
        );
